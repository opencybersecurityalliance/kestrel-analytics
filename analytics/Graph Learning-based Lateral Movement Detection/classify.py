...
class Classifier(object):
    ...
    def computeFeatureVector(self, data):
    	...
    	#adding time as new feature
    	features1=numpy.concatenate((F_src,F_dst,F_usr),axis=1)
    	data2=data.apply(lambda x: time.mktime(datetime.datetime.strptime(x['first_observed'],"%Y-%m-%dT%H:%M:%S.%fZ").timetuple()), axis=1) 
    ...
    def train_evaluate(self, data):
        ...
        updatedData=pd.concat([train_positive[['id','first_observed','user_id','src_ref.value','dst_ref.value','status']], train_negative[['id','first_observed','user_id','src_ref.value','dst_ref.value','status']],test[['id','first_observed','user_id','src_ref.value','dst_ref.value','status']]], ignore_index=True)

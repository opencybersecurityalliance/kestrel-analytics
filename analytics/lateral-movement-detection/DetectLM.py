...
def process(data):
    ...
    # I should use functions!!!
    data['timestamp']=data.apply(lambda x: time.mktime(datetime.datetime.strptime(x['first_observed'],"%Y-%m-%dT%H:%M:%S.%fZ").timetuple()), axis=1) 
    data['timeofweek'] = data.apply(lambda row: (row['timestamp'] // 3600 + 24 * 2) % 168, axis=1)
    # the number of seconds passed from midnight Monday of that week. 
    data['weekday'] = data.apply(lambda row: ((row['timestamp'] // 3600 + 24 * 2) % 168) // 24, axis=1)
    ...
    data['Tu'] = data.apply(lambda row: 1 if (row['timeofweek'] <48) and (row['timeofweek'] >=24) else 0, axis=1)
    data['We'] = data.apply(lambda row: 1 if (row['timeofweek'] <72) and (row['timeofweek'] >=48) else 0, axis=1)
    data['Th'] = data.apply(lambda row: 1 if (row['timeofweek'] <96) and (row['timeofweek'] >=72) else 0, axis=1)
    data['Fr'] = data.apply(lambda row: 1 if (row['timeofweek'] <120) and (row['timeofweek'] >=96) else 0, axis=1)
    data['Sa'] = data.apply(lambda row: 1 if (row['timeofweek'] <148) and (row['timeofweek'] >=120) else 0, axis=1)
    ...
    data['NoOfRecords'] = data.apply(lambda row: 1 , axis=1)
                                                         
    return data 

def sourceclustering(dfdata):
    sources=dfdata[['source','username']].groupby(['source'])['username'].nunique()
    temp=dfdata[['source','destination']].groupby(['source'])['destination'].nunique()
    sources=pd.merge(sources,temp, how='inner', on = ['source']).rename(columns={"username": "F1"})
    ...
    sources=pd.merge(sources,temp, how='inner', on = ['source'])
    temp=dfdata[['source','Days','id']].groupby(['source','Days'])['id'].count().reset_index().rename(columns={"id":"Count"})
    temp=temp.groupby(['source'])['Count'].mean().reset_index().rename(columns={"Count":"Mean"})
    sources=pd.merge(sources,temp, how='inner', on = ['source'])
    temp=dfdata[['source','destination','id']].groupby(['source','destination'])['id'].count().reset_index().rename(columns={"id":"Count"})
    temp=temp.groupby(['source'])['Count'].max().reset_index().rename(columns={"Count":"Max"})
    sources=pd.merge(sources,temp, how='inner', on = ['source'])
       
    mms = MinMaxScaler()
    mms.fit(sources[['F1','NoOfRecords','Mean','Max','Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su', '0to6', '6to12', '12to18', '18to0']])
    sources[['F1','NoOfRecords','Mean','Max','Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su', '0to6', '6to12', '12to18', '18to0']] = mms.transform(sources[['F1','NoOfRecords','Mean','Max','Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su', '0to6', '6to12', '12to18', '18to0']])
    kmeans = KMeans(n_clusters=ks,random_state=0)
    sources['c_src'] = kmeans.fit_predict( sources[['F1','NoOfRecords','Mean','Max','Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su', '0to6', '6to12', '12to18', '18to0']])
    
    return sources

def usernamedomainclustering(dfdata):
    usernames=dfdata[['username','domain','source']].groupby(['username','domain'])['source'].nunique()
    temp=dfdata[['username','domain','destination']].groupby(['username','domain'])['destination'].nunique()
    usernames=pd.merge(usernames,temp, how='inner', on = ['username','domain']).rename(columns={"source": "F1", "destination": "F2"})
    temp=dfdata[['username','domain','Mo']].groupby(['username','domain'])['Mo'].sum()
    usernames=pd.merge(usernames,temp, how='inner', on = ['username','domain'])
    temp=dfdata[['username','domain','Tu']].groupby(['username','domain'])['Tu'].sum()
    usernames=pd.merge(usernames,temp, how='inner', on = ['username','domain'])
    temp=dfdata[['username','domain','We']].groupby(['username','domain'])['We'].sum()
    usernames=pd.merge(usernames,temp, how='inner', on = ['username','domain'])
    temp=dfdata[['username','domain','Th']].groupby(['username','domain'])['Th'].sum()
    usernames=pd.merge(usernames,temp, how='inner', on = ['username','domain'])
    temp=dfdata[['username','domain','Fr']].groupby(['username','domain'])['Fr'].sum()
    usernames=pd.merge(usernames,temp, how='inner', on = ['username','domain'])
    temp=dfdata[['username','domain','Sa']].groupby(['username','domain'])['Sa'].sum()
    usernames=pd.merge(usernames,temp, how='inner', on = ['username','domain'])
    temp=dfdata[['username','domain','Su']].groupby(['username','domain'])['Su'].sum()
    usernames=pd.merge(usernames,temp, how='inner', on = ['username','domain'])
    temp=dfdata[['username','domain','NoOfRecords']].groupby(['username','domain'])['NoOfRecords'].sum()
    usernames=pd.merge(usernames,temp, how='inner', on = ['username','domain'])
    temp=dfdata[['username','domain','0to6']].groupby(['username','domain'])['0to6'].sum()
    usernames=pd.merge(usernames,temp, how='inner', on = ['username','domain'])
    temp=dfdata[['username','domain','6to12']].groupby(['username','domain'])['6to12'].sum()
    usernames=pd.merge(usernames,temp, how='inner', on = ['username','domain'])
    temp=dfdata[['username','domain','12to18']].groupby(['username','domain'])['12to18'].sum()
    usernames=pd.merge(usernames,temp, how='inner', on = ['username','domain'])
    temp=dfdata[['username','domain','18to0']].groupby(['username','domain'])['18to0'].sum()
    usernames=pd.merge(usernames,temp, how='inner', on = ['username','domain'])
    temp=dfdata[['username','domain','Days','id']].groupby(['username','domain','Days'])['id'].count().reset_index().rename(columns={"id":"Count"})
    temp=temp.groupby(['username','domain'])['Count'].mean().reset_index().rename(columns={"Count":"Mean"})
    usernames=pd.merge(usernames,temp, how='inner', on = ['username','domain'])
    temp=dfdata[['username','domain','destination','id']].groupby(['username','domain','destination'])['id'].count().reset_index().rename(columns={"id":"Count"})
    temp=temp.groupby(['username','domain'])['Count'].max().reset_index().rename(columns={"Count":"Max"})
    usernames=pd.merge(usernames,temp, how='inner', on = ['username','domain'])
       
    mms = MinMaxScaler()
    mms.fit(usernames[['F1','F2','NoOfRecords','Mean','Max','Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su', '0to6', '6to12', '12to18', '18to0']])
    usernames[['F1','F2','NoOfRecords','Mean','Max','Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su', '0to6', '6to12', '12to18', '18to0']] = mms.transform(usernames[['F1','F2','NoOfRecords','Mean','Max','Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su', '0to6', '6to12', '12to18', '18to0']])
    kmeans = KMeans(n_clusters=ku,random_state=0)
    usernames['c_usr'] = kmeans.fit_predict( usernames[['F1','F2','NoOfRecords','Mean','Max','Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su', '0to6', '6to12', '12to18', '18to0']])  
    
    return   usernames  
    
def usernameclustering(dfdata):
    usernames=dfdata[['username','source']].groupby(['username'])['source'].nunique()
    temp=dfdata[['username','destination']].groupby(['username'])['destination'].nunique()
    usernames=pd.merge(usernames,temp, how='inner', on = ['username']).rename(columns={"source": "F1", "destination": "F2"})
    ...
    usernames=pd.merge(usernames,temp, how='inner', on = ['username'])
    temp=dfdata[['username','NoOfRecords']].groupby(['username'])['NoOfRecords'].sum()
    usernames=pd.merge(usernames,temp, how='inner', on = ['username'])
    ...
    usernames=pd.merge(usernames,temp, how='inner', on = ['username'])
    temp=dfdata[['username','Days','id']].groupby(['username','Days'])['id'].count().reset_index().rename(columns={"id":"Count"})
    temp=temp.groupby(['username'])['Count'].mean().reset_index().rename(columns={"Count":"Mean"})
    usernames=pd.merge(usernames,temp, how='inner', on = ['username'])
    temp=dfdata[['username','destination','id']].groupby(['username','destination'])['id'].count().reset_index().rename(columns={"id":"Count"})
    temp=temp.groupby(['username'])['Count'].max().reset_index().rename(columns={"Count":"Max"})
    usernames=pd.merge(usernames,temp, how='inner', on = ['username'])
...
def destinationclustering(dfdata):
    destinations=dfdata[['destination','username']].groupby(['destination'])['username'].nunique()
    temp=dfdata[['destination','source']].groupby(['destination'])['source'].nunique()
    destinations=pd.merge(destinations,temp, how='inner', on = ['destination']).rename(columns={"username": "F1", "source": "F2"})
    ...
    destinations=pd.merge(destinations,temp, how='inner', on = ['destination'])
    temp=dfdata[['destination','NoOfRecords']].groupby(['destination'])['NoOfRecords'].sum()
    destinations=pd.merge(destinations,temp, how='inner', on = ['destination'])
    ...
    destinations=pd.merge(destinations,temp, how='inner', on = ['destination'])
    temp=dfdata[['destination','Days','id']].groupby(['destination','Days'])['id'].count().reset_index().rename(columns={"id":"Count"})
    temp=temp.groupby(['destination'])['Count'].mean().reset_index().rename(columns={"Count":"Mean"})
    destinations=pd.merge(destinations,temp, how='inner', on = ['destination'])
    temp=dfdata[['source','destination','id']].groupby(['source','destination'])['id'].count().reset_index().rename(columns={"id":"Count"})
    temp=temp.groupby(['destination'])['Count'].max().reset_index().rename(columns={"Count":"Max"})
    destinations=pd.merge(destinations,temp, how='inner', on = ['destination'])
       
    mms = MinMaxScaler()
    mms.fit(destinations[['F1','F2','NoOfRecords','Mean','Max','Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su', '0to6', '6to12', '12to18', '18to0']])
    destinations[['F1','F2','NoOfRecords','Mean','Max','Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su', '0to6', '6to12', '12to18', '18to0']] = mms.transform(destinations[['F1','F2','NoOfRecords','Mean','Max','Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su', '0to6', '6to12', '12to18', '18to0']])
    kmeans = KMeans(n_clusters=kd,random_state=0)
    destinations['c_dst'] = kmeans.fit_predict( destinations[['F1','F2','NoOfRecords','Mean','Max','Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su', '0to6', '6to12', '12to18', '18to0']])

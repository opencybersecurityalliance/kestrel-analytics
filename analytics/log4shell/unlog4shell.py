...
#Below tests cribbed from https://github.com/JasonKeirstead/log4shell-detector/blob/main/tests/test_detection.py
...
if __name__ == '__main__':
    for string_positive in TEST_STRINGS_POSITIVE:
        test_content = base64.b64decode(string_positive).decode("utf8")
        for line in test_content.splitlines():
            if not check_url(test_content):
                print("Test Case FAILURE, got negative match expected positive: " + line)

    for string_positive in TEST_STRINGS_POSITIVE_GZ:
        test_content = gzip.decompress(base64.b64decode(string_positive)).decode("utf8")
        for line in test_content.splitlines():
            if not check_url(test_content):
                print("Test Case FAILURE, got negative match expected positive: " + line)

    for string_negative in TEST_STRINGS_NEGATIVE:
        test_content = base64.b64decode(string_negative).decode("utf8")
        for line in test_content.splitlines():
